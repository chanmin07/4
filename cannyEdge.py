...
mag_map = np.zeros((ROI_GRAY.shape[0],ROI_GRAY.shape[1])) #그레이디언트 크기
dir_map = np.zeros((ROI_GRAY.shape[0],ROI_GRAY.shape[1]),dtype=np.uint8) #방향

gaussblurred = np.zeros((ROI_GRAY.shape[0],ROI_GRAY.shape[1])) #가우시안 미분적용될것

edge_map = np.zeros((ROI_GRAY.shape[0],ROI_GRAY.shape[1])) #엣지 결과 
visited = np.zeros((ROI_GRAY.shape[0],ROI_GRAY.shape[1])) #133p 방문한거는 안하려고
